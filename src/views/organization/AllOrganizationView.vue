<template>
  <div class="section__header">
    <div class="section__header__title">Все организации</div>
    <v-btn class="button_add" @click="dialog = !dialog">
      Добавить оргнаизация
    </v-btn>
  </div>
  ...
  >
      ...
      <v-dialog v-model="dialog" max-width="600px">
        <v-card ref="form">
          <div class="dialog__container">
            ...
            <div>
              <label class="universal__label">Тип организации</label>
              <v-autocomplete
                ref="category"
                required
    ...
    <template v-slot:[`item.name_ru`]="{ item }">
      <router-link
        to="/organization"
        style="text-decoration: none; color: #0075ff"
      >
        {{ item.raw.name_ru }}
      </router-link>
    </template>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Тип организации", key: "category" },
      { title: "Дата создания", key: "created_at" },
      { title: "Действие", key: "actions", sortable: false },
    ],
    dataItems: [],
    editedIndex: -1,
    categories: ["Гос. учреждение", "Частная компания", "ТОО"],
    editedItem: {
      name_kz: "",
      name_ru: "",
      category: null,
  ...
  methods: {
    initialize() {
      this.dataItems = [
        {
          name_kz: "ТОО КазМунайГаз в городе Жанаозен",
          name_ru: "ТОО КазМунайГаз в городе Жанаозен",
